...
struct B {
  B() { throw std::runtime_error("constructor error"); }
  B(int) {}
  void a_method() { throw std::runtime_error("method error"); }
};

int main() {
  qjs::Runtime runtime;
  qjs::Context context(runtime);
  context.global()["println"] = [](const std::string &s) {
    std::cout << s << std::endl;
  };
  ...
  try {
    ...
    assert(exc.isError() &&
           (std::string)exc == "TypeError: cannot convert symbol to number");
  }

  try {
    auto f = (std::function<void()>)context.eval(
  ...
  try {
    ...
    if ((bool)exc["stack"])
      std::cerr << (std::string)exc["stack"] << std::endl;
    assert(exc.isError() &&
           (std::string)exc == "ReferenceError: 'b' is not defined");
  }
  ...
  try {
    ...
    if ((bool)exc["stack"])
      std::cerr << (std::string)exc["stack"] << std::endl;
    assert(exc.isError() &&
           (std::string)exc == "InternalError: constructor error");
  }
