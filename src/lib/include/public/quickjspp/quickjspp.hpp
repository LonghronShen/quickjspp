 ...
 * Describes how to convert type R to/from JSValue. Second template argument can
 * be used for SFINAE/enable_if type filters.
 */
template <typename R, typename /*_SFINAE*/ = void> struct js_traits {
  /** Create an object of C++ type R given JSValue v and JSContext.
   * This function is intentionally not implemented. User should implement this
   * function for their own type.
   * @param v This value is passed as JSValueConst so it should be freed by the
   * caller.
   ...
   * This function is intentionally not implemented. User should implement this
   * function for their own type.
   * @return Returns JSValue which should be freed by the caller or JS_EXCEPTION
   * in case of error.
   */
 ...
 */
template <> struct js_traits<JSValue> {
  static JSValue unwrap(JSContext *ctx, JSValueConst v) noexcept {
    return JS_DupValue(ctx, v);
  }

  static JSValue wrap(JSContext *ctx, JSValue &&v) noexcept { return v; }
};

/** Conversion traits for integers.
 * Intentionally doesn't define traits for uint64_t since it can be typedefed to
 * JSValue. (@see JS_NAN_BOXING)
 */
template <typename Int>
struct js_traits<Int, std::enable_if_t<std::is_integral_v<Int> &&
 ...
 */
template <> struct js_traits<bool> {
 ...
 */
template <> struct js_traits<void> {
 ...
 */
template <> struct js_traits<double> {
  /// @throws exception
...
template <> struct js_traits<std::string_view> {
...
template <> struct js_traits<const char *> {
...
template <typename... Ts> struct js_traits<std::variant<Ts...>> {
  ...
  static std::optional<std::variant<Ts...>> unwrapImpl(JSContext *ctx,
                                                       JSValueConst v) {
  ...
  unwrapObj(JSContext *ctx, JSValueConst v, JSClassID class_id) {
    ...
    if constexpr (is_pair<U>::value) {
      if (JS_IsArray(ctx, v) == 1) {
        ...
        bool ok = isCompatible<std::decay_t<typename U::first_type>>(
                      ctx, firstElement) &&
                  isCompatible<std::decay_t<typename U::second_type>>(
                      ctx, secondElement);
        JS_FreeValue(ctx, firstElement);
   ...
   */
  template <template <typename T> typename Trait,
  ...
  static std::variant<Ts...> unwrapPriority(JSContext *ctx, JSValueConst v) {
    ...
    JS_ThrowTypeError(ctx, "Expected type %s",
  ...
  static bool isCompatible(JSContext *ctx, JSValueConst v) noexcept {
    ...
    case JS_TAG_BOOL:
      return is_boolean<T>::value || std::is_integral_v<T> ||
  ...
  static std::variant<Ts...> unwrap(JSContext *ctx, JSValueConst v) {
    ...
    case JS_TAG_BOOL:
      return unwrapPriority<is_boolean, std::is_integral,
                            std::is_floating_point>(ctx, v);
    ...
    case JS_TAG_UNINITIALIZED:
      [[fallthrough]];
    case JS_TAG_CATCH_OFFSET:
      JS_ThrowTypeError(ctx, "Expected type %s, got tag %d",
                        QJSPP_TYPENAME(std::variant<Ts...>), tag);
      [[fallthrough]];
    case JS_TAG_EXCEPTION:
      break;
...
template <typename T> struct rest : std::vector<T> {
...
template <typename T> T unwrap_free(JSContext *ctx, JSValue val) {
...
struct unwrap_arg_impl<rest<T>, I, NArgs> {
  static rest<T> unwrap(JSContext *ctx, int argc, JSValueConst *argv) {
    static_assert(I == NArgs - 1,
                  "The `rest` argument must be the last function argument.");
    rest<T> result;
...
Tuple unwrap_args_impl(JSContext *ctx, int argc, JSValueConst *argv,
                       std::index_sequence<I...>) {
  return Tuple{
      unwrap_arg_impl<std::tuple_element_t<I, Tuple>, I, sizeof...(I)>::unwrap(
          ctx, argc, argv)...};
}
 ...
 */
template <typename... Args>
std::tuple<std::decay_t<Args>...> unwrap_args(JSContext *ctx, int argc,
                                              JSValueConst *argv) {
  return unwrap_args_impl<std::tuple<std::decay_t<Args>...>>(
...
JSValue wrap_call(JSContext *ctx, Callable &&f, int argc,
  ...
  try {
    if constexpr (std::is_same_v<R, void>) {
      ...
      return js_traits<std::decay_t<R>>::wrap(
          ctx, std::apply(std::forward<Callable>(f),
                          unwrap_args<Args...>(ctx, argc, argv)));
    }
  } catch (exception) {
    return JS_EXCEPTION;
...
template <class Tuple, std::size_t... I>
void wrap_args_impl(JSContext *ctx, JSValue *argv, Tuple tuple,
                    std::index_sequence<I...>) {
  ((argv[I] = js_traits<std::decay_t<std::tuple_element_t<I, Tuple>>>::wrap(
        ctx, std::get<I>(tuple))),
   ...);
}
...
template <typename T> struct class_from_member_pointer {
  using type = void;
};
template <typename T, typename U> struct class_from_member_pointer<T U::*> {
  using type = U;
};
template <typename T>
 ...
 * @tparam F either a pointer to free function or a pointer to class member
 * function
 * @tparam PassThis if true and F is a pointer to free function, passes
 * Javascript "this" value as first argument:
 */
    ...
    return JS_NewCFunction(
        ...
        [](JSContext *ctx, JSValueConst this_value, int argc,
          ...
          if constexpr (PassThis)
            return detail::wrap_this_call<R, Args...>(ctx, F, this_value, argc,
                                                      argv);
          else
...
/** Conversion to JSValue for class member function in fwrapper. PassThis is
 * ignored and treated as true */
template <typename R, class T, typename... Args, R (T::*F)(Args...),
          bool PassThis /*=ignored*/>
struct js_traits<fwrapper<F, PassThis>> {
  static JSValue wrap(JSContext *ctx, fwrapper<F, PassThis> fw) noexcept {
    return JS_NewCFunction(
        ctx,
        [](JSContext *ctx, JSValueConst this_value, int argc,
           JSValueConst *argv) noexcept -> JSValue {
          return detail::wrap_this_call<R, std::shared_ptr<T>, Args...>(
              ctx, F, this_value, argc, argv);
        },
        fw.name, sizeof...(Args));
  }
};

/** Conversion to JSValue for const class member function in fwrapper. PassThis
 ...
 */
template <class T, typename... Args> struct ctor_wrapper {
 ...
 * @tparam T class type
 */
template <class T> struct js_traits<std::shared_ptr<T>> {
  ...
  using ptr_cast_fcn_t =
      std::function<std::shared_ptr<T>(JSContext *, JSValueConst)>;

  /// Used by registerDerivedClass to register new derived classes with this
  /// class' base type.
   ...
   */
  template <typename D>
  static void registerDerivedClass(JSClassID derived_class_id,
                                   ptr_cast_fcn_t ptr_cast_fcn) {
    static_assert(std::is_base_of<T, D>::value && !std::is_same<T, D>::value,
                  "Type is not a derived class");
    using derived_ptr_cast_fcn_t =
        typename js_traits<std::shared_ptr<D>>::ptr_cast_fcn_t;
    ...
    // Instrument the derived class so that it can propagate new derived classes
    // to us.
    ...
    js_traits<std::shared_ptr<D>>::registerWithBase =
        [old_registerWithBase = std::move(old_registerWithBase)](
            JSClassID derived_class_id,
            derived_ptr_cast_fcn_t derived_ptr_cast_fcn) {
          if (old_registerWithBase)
            old_registerWithBase(derived_class_id, derived_ptr_cast_fcn);
          registerDerivedClass<D>(
              derived_class_id,
  ...
  ensureCanCastToBase() {
    ...
    js_traits<std::shared_ptr<B>>::template registerDerivedClass<T>(QJSClassId,
                                                                    unwrap);
  }

  template <auto M> static void ensureCanCastToBase() {
    ensureCanCastToBase<detail::class_from_member_pointer_t<decltype(M)>>();
  }

  /** Stores offsets to qjs::Value members of T.
   * These values should be marked by class_registrar::mark for QuickJS garbage
   * collector so that the cycle removal algorithm can find the other objects
   ...
   * @param exotic pointer to QJS exotic methods(static lifetime) which allow
   * custom property handling. see quickjs doc
   * @throws exception
   */
    ...
    if (!JS_IsRegisteredClass(rt, QJSClassId)) {
      ...
      if (!markOffsets.empty()) {
        marker = [](JSRuntime *rt, JSValueConst val, JS_MarkFunc *mark_func) {
          auto pptr =
              static_cast<std::shared_ptr<T> *>(JS_GetOpaque(val, QJSClassId));
          assert(pptr);
      ...
      JSClassDef def{name,
                     ...
                     [](JSRuntime *rt, JSValue obj) noexcept {
                       auto pptr = static_cast<std::shared_ptr<T> *>(
                           JS_GetOpaque(obj, QJSClassId));
                       delete pptr;
                     },
                     // mark
                     marker,
   ...
   * Creates an object with class if #QJSClassId and sets its opaque pointer to
   * a new copy of #ptr.
   */
...
#else
      JS_ThrowTypeError(
          ctx, "quickjspp std::shared_ptr<T>::wrap: Class is not registered");
      return JS_EXCEPTION;
#endif
    }
...
/** Conversions for non-owning pointers to class T. nullptr corresponds to
 * JS_NULL.
 * @tparam T class type
 */
template <class T> struct js_traits<T *, std::enable_if_t<std::is_class_v<T>>> {
  static JSValue wrap(JSContext *ctx, T *ptr) {
    ...
    if (js_traits<std::shared_ptr<T>>::QJSClassId == 0) // not registered
    {
      ...
      // it probably means you are passing JSValueConst to where JSValue is
      // expected
      js_traits<std::shared_ptr<T>>::register_class(ctx, typeid(T).name());
#else
      JS_ThrowTypeError(
          ctx, "quickjspp js_traits<T *>::wrap: Class is not registered");
      return JS_EXCEPTION;
#endif
    }
    auto jsobj =
        JS_NewObjectClass(ctx, js_traits<std::shared_ptr<T>>::QJSClassId);
    if (JS_IsException(jsobj))
      return jsobj;
...
template <typename E> struct js_traits<E, std::enable_if_t<std::is_enum_v<E>>> {
 ...
 */
struct function {
  JSValue (*invoker)(function *self, JSContext *ctx, JSValueConst this_value,
                     int argc, JSValueConst *argv) = nullptr;
  ...
  static function *create(JSRuntime *rt, Functor &&f) {
    ...
    auto fptr = static_cast<function *>(
        js_malloc_rt(rt, sizeof(function) + sizeof(Functor_t)));
    if (!fptr)
      throw std::bad_alloc{};
    new (fptr) function;
...
template <> struct js_traits<detail::function> {
  inline static JSClassID QJSClassId = 0;
  ...
  static void register_class(JSContext *ctx, const char *name) {
    ...
    JSClassDef def{
        ...
        [](JSRuntime *rt, JSValue obj) noexcept {
          auto fptr =
              static_cast<detail::function *>(JS_GetOpaque(obj, QJSClassId));
          assert(fptr);
          if (fptr->destroyer)
            fptr->destroyer(fptr);
          js_free_rt(rt, fptr);
        ...
        nullptr, // mark
        // call
        [](JSContext *ctx, JSValueConst func_obj, JSValueConst this_val,
           int argc, JSValueConst *argv, int flags) -> JSValue {
          auto ptr = static_cast<detail::function *>(
              JS_GetOpaque2(ctx, func_obj, QJSClassId));
          if (!ptr)
            return JS_EXCEPTION;
 ...
 */
template <typename Key> struct js_property_traits {
  static void set_property(JSContext *ctx, JSValue this_obj, Key key,
                           JSValue value);

  static JSValue get_property(JSContext *ctx, JSValue this_obj, Key key);
};

template <> struct js_property_traits<const char *> {
  static void set_property(JSContext *ctx, JSValue this_obj, const char *name,
                           JSValue value) {
...
template <> struct js_property_traits<int> : js_property_traits<uint32_t> {};

namespace detail {
template <typename Key> struct property_proxy {
  JSContext *ctx;
  JSValue this_obj;
  Key key;
  ...
  template <typename T> T as() const {
    return unwrap_free<T>(
        ctx, js_property_traits<Key>::get_property(ctx, this_obj, key));
  }
  ...
  template <typename T> property_proxy &operator=(T &&value) {
    js_property_traits<Key>::set_property(
        ctx, this_obj, key,
        js_traits<std::decay_t<T>>::wrap(ctx, std::forward<T>(value)));
    return *this;
  }

  template <typename Key2> property_proxy<Key2> operator[](Key2 key2) const {
...
template <auto M> struct get_set {};

// M -  member object
template <class T, typename R, R T::*M> struct get_set<M> {
  using is_const = std::is_const<R>;

  static const R &get(std::shared_ptr<T> ptr) { return *ptr.*M; }
...
template <typename R, R *M> struct get_set<M> {
  using is_const = std::is_const<R>;
 ...
 * A JSValue can be released by either JSValue x = std::move(value); or JSValue
 * x = value.release(), then the Value becomes invalid and FreeValue won't be
 * called Can be converted to C++ type, for example: auto string =
 * value.as<std::string>(); qjs::exception would be thrown on error Properties
 * can be accessed (read/write): value["property1"] = 1; value[2] = "2";
 */
class Value {
public:
  JSValue v;
...
public:
  ...
  template <typename T> Value(JSContext *ctx, T &&val) : ctx(ctx) {
  ...
  /** Returns true if 2 values are the same (equality for arithmetic types or
   * point to the same object) */
  bool operator==(const Value &rhs) const { return (*this == rhs.v); }
  ...
  /** Implicit conversion to JSValue (rvalue only). Example: JSValue v =
   * std::move(value); */
  operator JSValue() &&noexcept { return release(); }

  /** Access JS properties. Returns proxy type which is implicitly convertible
   * to qjs::Value */
  template <typename Key> detail::property_proxy<Key> operator[](Key key) {
    assert(ctx && "Trying to access properties of Value with no JSContext");
    return {ctx, JS_DupValue(ctx, v), std::move(key)};
  }

  // add("f", []() {...});
  template <typename Function> Value &add(const char *name, Function &&f) {
    (*this)[name] =
        js_traits<decltype(std::function{std::forward<Function>(f)})>::wrap(
  ...
  template <auto FGet, auto FSet> Value &add_getter_setter(const char *name) {
    ...
    int ret = JS_DefinePropertyGetSet(
        ctx, v, prop, js_traits<fgetter>::wrap(ctx, fgetter{name}),
  ...
  template <auto FGet> Value &add_getter(const char *name) {
    auto prop = JS_NewAtom(ctx, name);
    using fgetter = fwrapper<FGet, true>;
    int ret = JS_DefinePropertyGetSet(
        ctx, v, prop, js_traits<fgetter>::wrap(ctx, fgetter{name}),
  ...
  add(const char *name) {
    if constexpr (detail::get_set<M>::is_const::value)
      ...
      return add_getter_setter<detail::get_set<M>::get,
                               detail::get_set<M>::set>(name);
  }

  // add<&T::static_member>("static_member");
  template <auto M>
  std::enable_if_t<std::is_pointer_v<decltype(M)> &&
                       !std::is_function_v<std::remove_pointer_t<decltype(M)>>,
                   Value &>
  add(const char *name) {
    if constexpr (detail::get_set<M>::is_const::value)
      return add_getter<detail::get_set<M>::get>(name);
  ...
  std::string toJSON(const Value &replacer = JS_UNDEFINED,
                     const Value &space = JS_UNDEFINED) {
    assert(ctx);
  ...
  Value evalThis(std::string_view buffer, const char *filename = "<evalThis>",
                 int flags = 0) {
...
public:
  ...
  Runtime() {
    ...
    JS_SetHostPromiseRejectionTracker(rt, promise_unhandled_rejection_tracker,
                                      NULL);
  ...
  /// @return pointer to qjs::Context of the executed job or nullptr if no job
  /// is pending
...
public:
  ...
  public:
    Module(JSContext *ctx, const char *name) : ctx(ctx), name(name) {
      m = JS_NewCModule(ctx, name, [](JSContext *ctx, JSModuleDef *m) noexcept {
        ...
        auto it =
            std::find_if(context.modules.begin(), context.modules.end(),
     ...
     */
    template <auto F> Module &function(const char *name) {
     ...
     */
    template <typename F> Module &function(const char *name, F &&f) {
      return add(name,
                 js_traits<decltype(std::function{std::forward<F>(f)})>::wrap(
                     ctx, std::forward<F>(f)));
    }
  ...
  private:
     ...
     */
    template <class T> class class_registrar {
      const char *name;
      qjs::Value prototype;
    ...
    public:
      explicit class_registrar(const char *name, qjs::Context::Module &module,
                               qjs::Context &context)
          : name(name), prototype(context.newObject()), module(module),
            context(context), ctor(JS_NULL) {}

      class_registrar(const class_registrar &) = delete;

      /** Add functional object f
       */
      template <typename F> class_registrar &fun(const char *name, F &&f) {
       ...
       */
      template <auto F> class_registrar &fun(const char *name) {
       ...
       */
      template <auto F> class_registrar &static_fun(const char *name) {
        assert(!JS_IsNull(ctor.v) &&
               "You should call .constructor before .static_fun");
       ...
       * @tparam B base class
       */
      template <class B> class_registrar &base() {
        static_assert(!std::is_same_v<B, T>, "Type cannot be a base of itself");
        assert(js_traits<std::shared_ptr<B>>::QJSClassId &&
               "base class is not registered");
        js_traits<std::shared_ptr<T>>::template ensureCanCastToBase<B>();
        auto base_proto = JS_GetClassProto(
            context.ctx, js_traits<std::shared_ptr<B>>::QJSClassId);
      ...
      /** All qjs::Value members of T should be marked by mark<> for QuickJS
       * garbage collector so that the cycle removal algorithm can find the
       * other objects referenced by this object.
       */
      template <Value T::*V> class_registrar &mark() {
  ...
  public:
     ...
     */
    template <class T> class_registrar<T> class_(const char *name) {
...
public:
  ...
  Context(JSContext *ctx) : ctx{ctx} { init(); }
  ...
  struct ModuleData {
    ...
    ModuleData(std::optional<std::string> url,
  ...
  template <typename Function> void enqueueJob(Function &&job);
  ...
  /// @see JS_Eval
  Value eval(std::string_view buffer, const char *filename = "<eval>",
             int flags = 0) {
    assert(buffer.data()[buffer.size()] == '\0' &&
           "eval buffer is not null-terminated"); // JS_Eval requirement
  ...
  Value evalFile(const char *filename, int flags = 0) {
    ...
    if (!buf)
      throw std::runtime_error{std::string{"evalFile: can't read file: "} +
                               filename};
  ...
  Value fromJSON(std::string_view buffer, const char *filename = "<fromJSON>",
    ...
    return Value{
        ctx, JS_ParseJSON2(ctx, buffer.data(), buffer.size(), filename, flags)};
  }
...
struct js_traits<std::function<R(Args...)>, int> {
  ...
  static JSValue wrap(JSContext *ctx, Functor &&functor) noexcept {
    ...
    try {
      auto fptr =
          function::create(JS_GetRuntime(ctx), std::forward<Functor>(functor));
      fptr->invoker = [](function *self, JSContext *ctx,
...
template <typename T>
struct is_callable<
    T, std::enable_if_t<std::is_same_v<decltype(void(&T::operator())), void>>>
    : std::true_type {};
...
/** Convert from std::vector<T> to Array and vice-versa. If Array holds objects
...
template <typename U, typename V> struct js_traits<std::pair<U, V>> {
  ...
  static std::pair<U, V> unwrap(JSContext *ctx, JSValueConst jsarr) {
    ...
    if (len != 2) {
      JS_ThrowTypeError(
          ctx,
          "js_traits<%s>::unwrap expected array of length 2, got length %d",
 ...
 */
template <typename T> struct js_traits<std::optional<T>> {
  /** Wraps T or null. */
...
namespace detail {
template <typename Key> property_proxy<Key>::operator Value() const {
  return as<Value>();
}
} // namespace detail
...
inline JSModuleDef *
Runtime::module_loader(JSContext *ctx, const char *module_name, void *opaque) {
  ...
  try {
    ...
    // compile the module
    auto func_val =
        context.eval(*data.source, module_name,
                     JS_EVAL_TYPE_MODULE | JS_EVAL_FLAG_COMPILE_ONLY);
    assert(JS_VALUE_GET_TAG(func_val.v) == JS_TAG_MODULE);
    JSModuleDef *m =
        reinterpret_cast<JSModuleDef *>(JS_VALUE_GET_PTR(func_val.v));
